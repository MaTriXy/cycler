...
enum class SwipeDirection(val absolute: Boolean, val platformValue: Int) {
  ...
  companion object {
    val NONE = EnumSet.noneOf(SwipeDirection::class.java)
    val BOTH = EnumSet.of(START, END)
    val BOTH_ABSOLUTE = EnumSet.of(LEFT, RIGHT)
  }
}
...
class MutationExtensionSpec<T : Any> : ExtensionSpec<T> {
  ...
  internal var canSwipeItem: ((T) -> EnumSet<SwipeDirection>) = { SwipeDirection.NONE }
  ...
  /**
   * Register a callback that will be invoked when an item is removed by swiping it.
   * @param block receives the swiped data item and the direction. Direction is one of
   ...
   */
  fun canSwipe(block: (T) -> EnumSet<SwipeDirection>) {
    canSwipeItem = block
  }
...
internal constructor(
  val dataItem: T,
  val swipedView: View,
  private val allowedDirections: EnumSet<SwipeDirection>
) {
...
private class MutationExtension<T : Any>(val spec: MutationExtensionSpec<T>) : Extension<T> {
  ...
  private inner class MyItemTouchCallback : ItemTouchHelper.Callback() {
    ...
    private fun createSwipeStateFor(
      dataItem: T,
      itemView: View,
      allowedDirections: EnumSet<SwipeDirection>
    ) {
      ...
      // CreatorContext(recycler) is provided.
      // Not perfect because that provides access to extensions in the main recycler view
      // and this is not supposed to live in the same level as the main recycler rows definition.
      // To be strict CreatorContext should be an interface so we can provide 'no extensions'.
    ...
    override fun onSwiped(
      ...
      // Tell the callback.
      spec.onSwiped?.invoke(data.data[position], direction.toSwipeDirection())
    }
  }
}

private fun EnumSet<SwipeDirection>.toPlatformValue() = fold(0) { acc, direction ->
